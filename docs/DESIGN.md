...
| 26 | 構造化出力の取得方式 | 出力用モデルをエージェントループのツールとして渡す方式（`agent(prompt, structured_output_model=...)`）で取得し、失敗は初回 + リトライ 2 回まで、4 秒 → 16 秒の間隔を空けて再試行する（従来の「非推奨の `structured_output()` を待機なしで 3 回」を改める） | 旧方式は調査中のツール呼び出しが残った会話履歴に対して出力用ツールだけを提示するため、モデルが調査ツールの名前を返すとその場で失敗していた。ループ内なら余分なツール呼び出しは実行されて先に進む。待機なしの連打は 5 秒で 3 回を使い切り、事実上リトライになっていなかった。新方式は呼び出しのたびに会話履歴へ追記して失敗しても巻き戻さないため、再試行の前に履歴を巻き戻さないと user メッセージの連続などで 2 回目以降が確定的に失敗する（旧方式の「履歴を変更しないので再試行は安全」は成り立たなくなった） |
...
| 29 | 失敗の通知と例外の扱い | エージェント内で捕捉できた失敗は Slack へ通知したうえで正常終了させ（`run_daily_check` / `run_adhoc_investigation` は `None` を返す）、通知すら発行できなかった失敗だけを例外として送出する。日次には専用の失敗通知（`build_daily_failure_notification`）を新設し、載せるのはエラー内容と対処の案内のみ | 日次パスには失敗通知が無く、失敗すると Slack に何も出ないまま終わっていた。決定 10（毎日必ず 1 通・死活確認を兼ねる）を失敗した日にも守るため。通知済みの失敗まで例外にすると、決定 30 のアラームと合わせて Slack に 2 通届いてしまう |
| 30 | 通知できない失敗の検知 | 中継 Lambda の `Errors` メトリクスに CloudWatch アラーム（5 分・1 データポイント・`Errors >= 1`・欠損は `notBreaching`）を張り、同じ SNS トピックへ通知する。OK 復帰の通知は付けない | コンテナ障害やタイムアウトはエージェント側の `try/except` では捕捉できず、決定 29 だけでは無言の失敗が残るため。復旧は翌日の日次通知が届くことで分かる。決定 12 が除外しているのは AgentCore Observability であり、これには抵触しない |
