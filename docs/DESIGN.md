...
| 28 | プロンプトの構成 | 静的な方針・基準はスキルへ切り出す（採点基準 = `scoring-rubric`、調査の方針・手順 = `investigation-policy`。従来の「採点基準はシステムプロンプトに明記」を改める）。スキル本文は保証注入ではないため、調査プロンプトの冒頭で両スキルの読み込みを明示指示する。設定値の埋め込み・インジェクション対策のガード文言・時刻の扱いはコード側に残し、プロンプト組み立ては `prompts.py` に集約する | プロンプトの点在を解消し、方針・基準の文言をコードから分離して編集しやすくするため。ガード文言は確実に注入される経路に置かないとセキュリティ設計が崩れるため。書式スキルとの時刻ルールの重複は、出力直前の最終防衛線として意図的に残す |

## スコアリング方式

- エージェントは調査終了後、問題ごとに 0〜100 点のスコアと根拠を構造化出力（Pydantic モデル）で返す
- 採点基準（影響範囲・緊急度・継続性）は `scoring-rubric` スキルに定義し、調査前に読み込ませる（決定 28）
